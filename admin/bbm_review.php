...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['approve_id'])) {
        ...
        $ok = adminApproveSubmission('BBM', $id, $admin_id);
        if ($ok) $message = 'Pengajuan BBM disetujui.';
        else $error = 'Gagal menyetujui pengajuan.';
    }
    if (isset($_POST['reject_id'])) {
        $id = intval($_POST['reject_id']);
        $admin_id = $_SESSION['user']['id'];
        $catatan = $_POST['catatan'] ?? '';
        $ok = adminRejectSubmission('BBM', $id, $admin_id, $catatan);
        if ($ok) $message = 'Pengajuan BBM ditolak.';
        else $error = 'Gagal menolak pengajuan.';
    }
}

$pending = getAdminPendingSubmissions('bbm', 200);
?>
<!DOCTYPE html>
<html lang="id">
<head>
            ...
            <?php if(empty($pending)): ?>
                <tr>
                    <td colspan="9" style="text-align: center; padding: 40px; color: #999;">
                        📭 Tidak ada pengajuan BBM yang pending
                    </td>
                </tr>
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td>
                        <?php if (!empty($row['foto_struk'])): ?>
                            <a href="../uploads/struk_bbm/<?php echo htmlspecialchars($row['foto_struk']); ?>" 
                               target="_blank" class="btn btn-sm btn-info">👁️ Lihat</a>
                        <?php else: ?>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            <textarea name="catatan" id="catatan" class="form-control" rows="4" required 
